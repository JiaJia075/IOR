...
from inversion_tools.inversion_utils import xyxy2xywh, xywh2xyxy
...
def load_batch(train_txt_path, order_class_path, batch_size=64, img_size=320, shuffle=False):
    ...
    dataset = LoadImagesAndLabels(train_txt_path, img_size, batch_size,
                order_class_path=order_class_path, augment=False, hyp=hyp, rect=False, cache_images=False,
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=416, batch_size=16, order_class_path=None, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        self.order_class_path = order_class_path
        ...
        cat_ids_path = self.order_class_path
        self.cat_ids = []
        with open(cat_ids_path, 'r', encoding='utf-8') as file:
            lines = file.readlines()
            for line in lines:
                value_num = line.split(',')
                ids = int(value_num[1])
                self.cat_ids.append(ids)
            ...
            if os.path.isfile(label_path):
                ...
                if x.size > 0:
                    ...
                    for id in range(ids.shape[0]):
                        labels[id, 0] = self.cat_ids.index(ids[id])

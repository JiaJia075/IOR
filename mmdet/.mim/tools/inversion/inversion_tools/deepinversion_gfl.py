...
import collections
from inversion_tools.inversion_utils import create_folder
...
from inversion_tools.inversion_utils import flip_targets, jitter_targets, random_erase_masks, inference, draw_targets
from inversion_tools.inversion_utils import xywh2xyxy
...
class DeepInversionClass(object):
    def __init__(self, 
        ...
        for module in self.net_teacher.modules():
            if isinstance(module, nn.BatchNorm2d):
                self.loss_r_feature_layers.append(DeepInversionFeatureHook(module, p_norm=self.p_norm))
    ...
    def get_images(self, targets, init):
        ...
        for iteration in range(1,self.iterations+1):
            ...
            outputs = net_teacher(inputs_jit)#获得模型输入
            ...
            targets_jit_label = targets_jit[:,1:2]
            targets_jit_bbox = targets_jit[:,2:6]
            targets_jit_bbox[:, [1, 3]] *= init.size()[3]  # height
            targets_jit_bbox[:,[0, 2]] *= init.size()[2]  # width
            targets_jit_bbox = xywh2xyxy(targets_jit_bbox)

            for batch in range(init.size()[0]):
                batch_gt_instance = InstanceData()
                batch_img_meta = {}

                batch_gt_instance["bboxes"] = torch.Tensor([[targets_jit_bbox[batch,0],
                                                            targets_jit_bbox[batch,1],
                                                            targets_jit_bbox[batch,2],
                                                            targets_jit_bbox[batch,3]]]).cuda()
                # batch_gt_instance["bboxes"] = targets_jit_bbox[batch,:]
                # batch_gt_instance["labels"] = targets_jit_label[batch,:]
                batch_gt_instance["labels"] = torch.Tensor([targets_jit_label[batch]]).to(torch.int64).cuda()

                batch_img_meta['img_shape'] = (init.size()[2], init.size()[3])
                batch_img_meta['pad_shape'] = (init.size()[2], init.size()[3])

                batch_gt_instances.append(batch_gt_instance)
                batch_img_metas.append(batch_img_meta)

            loss_inputs = outputs + (batch_gt_instances, batch_img_metas)


            # R_cross classification loss NOTE: MODIFIED FOR YOLO
            task_loss_all = net_teacher.bbox_head.loss_by_feat(*loss_inputs)
            task_loss_cls = torch.stack(task_loss_all['loss_cls']).sum()
            task_loss_bbox = torch.stack(task_loss_all['loss_bbox']).sum()
            task_loss_dfl = torch.stack(task_loss_all['loss_dfl']).sum()
            # task_loss_copy_cls = task_loss_cls.clone().detach()
            # task_loss_copy_bbox = task_loss_bbox.clone().detach()
            # task_loss_copy_dfl_dfl = task_loss.clone().detach()

            task_loss = task_loss_cls + task_loss_bbox + task_loss_dfl
            ...
            # self.writer.add_scalar("unweighted/task_loss_cls", task_loss_cls.item() , iteration)
            # self.writer.add_scalar("unweighted/task_loss_bbox", task_loss_bbox.item() , iteration)
            # self.writer.add_scalar("unweighted/task_loss_gfl", task_loss_dfl.item() , iteration)
            ...
            if (iteration % self.display_every) == 0:
                ...
                print("[WEIGHTED] task loss", task_loss.item())
                print("[WEIGHTED] task loss cls", task_loss_cls.item())
                print("[WEIGHTED] task loss bbox", task_loss_bbox.item())
                print("[WEIGHTED] task loss gfl", task_loss_dfl.item())
            ...
            if (iteration % self.save_every) == 0: 
                ...
                self.save_image(im_copy,os.path.join(self.path, "iteration_targets_{}.jpg".format(iteration)), halfsize=False)

                # del im_copy, im_boxes_teach, im_boxes_verif, imgs_with_boxes_targets
                # del im_copy, imgs_with_boxes_targets
                del im_copy

...
def quality_focal_loss(pred, target, beta=2.0):
    ...
    scale_factor = scale_factor.abs().mean(dim=0, keepdim=False)
...
class QualityFocalLoss(nn.Module):
    ...
    def forward(self,
        ...
        if loss_weight != None:
            self.loss_weight=loss_weight*self.loss_weight
        reduction = (
        ...
        if self.use_sigmoid:
            ...
            loss_cls, scale_factor = calculate_loss_func(
                pred,
                target,
                weight,
                beta=self.beta,

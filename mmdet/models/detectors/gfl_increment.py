...
class GFLIncrement(GFL):
    ...
    def sel_pos_single(self, cat_cls_scores, cat_bbox_preds, batch_data_samples):
        ...
        _, top_indices = torch.topk(max_scores, k=topk)
        valid_mask = torch.full_like(max_scores, False, dtype=torch.bool)
        ...
        _, top_indices = torch.topk(max_bbox, k=topk)
        bbox_valid_mask = torch.full_like(max_bbox, False, dtype=torch.bool)
        bbox_valid_mask[top_indices] = True
    ...
    def sel_pos(self, cls_scores, bbox_preds, batch_data_samples):
    ...
    def loss(self, batch_inputs: Tensor, batch_data_samples: SampleList) -> Union[dict, list]:
        ...
        ori_mid_feature = self.ori_model.extract_feat(batch_inputs)
        ori_outs = self.ori_model.bbox_head(ori_mid_feature)


        # select positive predictions from original model
        topk_cls_inds, topk_cls_scores, topk_bbox_inds, topk_bbox_preds = self.sel_pos(ori_outs[0], ori_outs[1], batch_data_samples)


        # get new model outputs
        new_mid_feature = self.extract_feat(batch_inputs)
        new_outs = self.bbox_head(new_mid_feature)

        # calculate losses including general losses of new model and distillation losses of original model
        loss_inputs = (ori_outs, new_outs, ori_mid_feature, new_mid_feature, batch_data_samples, \
